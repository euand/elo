...
class League():
    ...
    def update(self, player, exp, score):
        self.player_scores[player] = self.player_scores[player] + 32 * (score - exp)
        return self
    ...
    def match(self, match_info):
        ...
        if home_score > away_score:
            self = self.update(home_player, exp, 1 + (1-exp)*(home_score - away_score)/20)
            self = self.update(away_player, (1-exp), 0 + exp*(away_score - home_score + overtime/2)/20)

        if away_score > home_score:
            self = self.update(home_player, exp, 0 + (1-exp)*(home_score - away_score + overtime/2)/20)
            self = self.update(away_player, (1-exp), 1 + exp*(away_score - home_score)/20)

        return self

    def load(self, player_scores):
        self.player_scores = player_scores
        self.players = list(player_scores.keys())
